...
//		}
	...
	public List<WorkflowNode> materializeExghaustive(MaterializedWorkflow1 materializedWorkflow,
										  Workflow1DPTable dpTable, String fromName) throws Exception {
		...
		if( !isOperator){
			...
			if(temp!=null){
				...
				dpTable.addRecord(temp.dataset, plan, temp.computePolicyFunction(metrics,
						materializedWorkflow.function),metrics);
				return ret;
			}
		}
		...
		if(isOperator){
			if(isAbstract){
				...
				for(Operator op : operators){
					...
					logger.info("Materialized operator: " + op.opName);
					List<WorkflowNode> bestInputs = new ArrayList<WorkflowNode>();
					//TODO: recursively iterate inputs
					List<List<WorkflowNode>> existingPlans = retrievePlans(op, materializedInputs, dpTable);

					//iterate inputs
					// gia ka8e apotelesma, materialize to ret
